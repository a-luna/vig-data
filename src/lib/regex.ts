...
export const BBREF_GAME_ID_REGEX = /(?<home_team_id>[A-Z]{3,3})(?<year>\d{4,4})(?<month>\d{2,2})(?<day>\d{2,2})(?<game_number_this_day>\d)/;
export const BB_GAME_ID_REGEX = /gid_(?<year>\d{4,4})_(?<month>\d{2,2})_(?<day>\d{2,2})_(?<away_team>[a-z]+)mlb_(?<home_team>[a-z]+)mlb_(?<game_num>\d)/;
