<script lang="ts">
	import { getStandingsOnDate } from '$lib/api/season';
	...
	async function getStandings(): Promise<ApiResponse<SeasonData>> {
		getStandingsResult = await getStandingsOnDate($mostRecentScrapedDate);
	...
	$: if ($mostRecentScrapedDate) {
		getStandingsRequest = getStandings();
		season = $mostRecentScrapedDate.getFullYear();
	}
</script>
