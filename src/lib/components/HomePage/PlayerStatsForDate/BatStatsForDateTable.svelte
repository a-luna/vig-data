<script lang="ts">
	...
	import { formatPosNegValue, getDummyPlayerBatStats, getHomeTeamIdFromGameId } from '$lib/util';
	...
	$: sortedBatStats = batStats.sort(getSortFunction(getDummyPlayerBatStats(), sortBy, sortDir));
		...
		<div class="resp-table-wrapper">
			<div id={tableId} class="text-sm leading-5 table-fixed resp-table">
				<div class="text-center resp-table-header col-header">
					...
					>
					<div class="table-header-cell" data-stat-name="stat_line" style={`width: ${statLineColWidth}px`}>
						Stat Line
					</div>
				</div>
